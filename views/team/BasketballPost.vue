  ...
  <div class="column">
    <article v-for="(item, index) in articleList" :key="index" class="media">
      <div class="media-left">
        <figure>
          <img style="width: 200px" src="../../assets/image/basketball.png" />
        </figure>
      </div>
        ...
        <nav class="level has-text-grey is-mobile is-size-6 mt-2">
          <div class="level-left">
            <div class="level-left">
              ...
              <span class="is-hidden-mobile" style="margin-left: 7px"
                >组队人数:{{ item.candidates }}/10人</span
              >
            </div>
          </div>
          <div v-if="item.candidates <= 9" class="media-right">
            <b-button class="btn-animate" @click="jointhem(item.id, index)"
              >加我一个!</b-button
            >
          </div>
          <div v-else>
...
export default {
  name: "BasketballPost",
  components: { Pagination, carousel2 },
  data() {
    return {
      activeName: "latest",
      joinornot: 0,
  ...
  methods: {
    ...
    jointhem(id, index) {
      ...
      if (this.articleList[index].candidates > 9) {
        alert("超过最大人数");
      } else {
